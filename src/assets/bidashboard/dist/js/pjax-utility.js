...
$(function () {
    ...
    var modalPjax = $('#modal-pjax-id');
    modalPjax.on('show.bs.modal', function (e) {
        var button = $(e.relatedTarget);
        modalPjax.find('.modal-dialog').addClass(button.data('size'));
        modalPjax.find('#modalPjaxHeader-id').html('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4>' + button.data('title') + '</h4>');
    });
    ...
    modalPjax.on('shown.bs.modal', function (e) {
        var button = $(e.relatedTarget),
            ...
            pjaxSettings = {
                ...
                url: button.data('url'),
                container: '#modalPjaxContent-id'
            };

        handleFormSubmit = handleFormSubmit !== undefined ? handleFormSubmit : true;
        $('#modalPjaxContent-id').off('pjax:error');
        $('#modalPjaxContent-id').on('pjax:error', function (event, xhr, textStatus, error, options) {
        ...
        $('#modalPjaxContent-id').off('pjax:complete');
        $('#modalPjaxContent-id').on('pjax:complete', function (event, xhr, textStatus, options) {
    ...
    modalPjax.on('hidden.bs.modal', function (e) {
        $(this).find('#modalPjaxContent-id').html('<div class="d-flex justify-content-center align-items-center" style="min-height:150px;"> <div class="spinner-grow text-mobit p-3" role="status"> <span class="sr-only">Loading ...</span> </div> </div>');
    });

    // Pjax Over Modal

    var modalPjaxOver = $('#modal-pjax-over-id');
    modalPjaxOver.on('show.bs.modal', function (e) {
        var button = $(e.relatedTarget);
        modalPjaxOver.find('.modal-dialog').addClass(button.data('size'));
        modalPjaxOver.find('#modalPjaxOverHeader-id').html('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4>' + button.data('title') + '</h4>');
    });
    ...
    modalPjaxOver.on('shown.bs.modal', function (e) {
        var button = $(e.relatedTarget),
            ...
            pjaxSettings = {
                ...
                skipOuterContainers: true,
                url: button.data('url'),
                container: '#modalPjaxOverContent-id'
            };

        handleFormSubmit = handleFormSubmit !== undefined ? handleFormSubmit : true;
        $('#modalPjaxOverContent-id').off('pjax:error');
        $('#modalPjaxOverContent-id').on('pjax:error', function (event, xhr, textStatus, error, options) {
        ...
        $('#modalPjaxOverContent-id').off('pjax:complete');
        $('#modalPjaxOverContent-id').on('pjax:complete', function (event, xhr, textStatus, options) {
            // ==============================================================
            //tooltip
            // ==============================================================
            $(function () {
        ...
        if (reloadUnderPjaxContainer !== undefined) {
            ...
            $("#modal-pjax-id").css({overflow: "auto"});
        }
        $.pjax(pjaxSettings);
    });

    modalPjaxOver.on('hidden.bs.modal', function (e) {
        $(this).find('#modalPjaxOverContent-id').html('<div class="d-flex justify-content-center align-items-center" style="min-height:150px;"> <div class="spinner-grow text-mobit p-3" role="status"> <span class="sr-only">Loading ...</span> </div> </div>');
    });
